...
description: Use toolbox github app-auth with gh without exposing temporary GitHub App installation tokens unnecessarily. Use when an agent or automation needs short-lived GitHub repository access through app credentials for issues, pull requests, releases, or repository API calls, and must avoid personal access tokens, persistent gh login, shell history leaks, or accidental token logging.
---

# GitHub App Auth

Use the `toolbox` binary to sign a GitHub App JWT and exchange it for an
installation access token, then pass that token to `gh` through a short-lived
environment variable. Prefer this skill when GitHub access should come from a
...
Run one `gh` command with a token scoped to the repository being accessed:
...
Run a short sequence of `gh` commands in one subshell:
...
Use the token for a direct GitHub API call through `gh api`:
...
CI step where logs are masked and shell tracing is disabled:
